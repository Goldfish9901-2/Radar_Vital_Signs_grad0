"""Smoke validation for Mamba training (no architecture changes intended).

Runs one epoch on the real exported dataset and reports:
...
Usage:
  uv run python validate_mamba_smoke.py

Note: batch size is kept small (16) because the pure-PyTorch selective scan
materializes a (B, L, d_inner, d_state) tensor per step; the 4 GB laptop GPU
would otherwise risk OOM at the default Conv-style batch size of 64.
"""
...
from src.models.mamba import MambaConfig, MambaHeartRateModel, count_parameters
...
def main() -> None:
    ...
    cfg = MambaConfig()  # use_frequency_domain defaults True
    model = MambaHeartRateModel(cfg).to(DEVICE)
    ...
    # --- [1] one epoch + [2] loss decrease + [3] NaN ---
    model.train()
    losses, maes = [], []
    nan_seen = False
    if DEVICE.type == "cuda":
    ...
    print(f"[1] epoch batches: {n_batches}  time: {elapsed:.1f}s")
