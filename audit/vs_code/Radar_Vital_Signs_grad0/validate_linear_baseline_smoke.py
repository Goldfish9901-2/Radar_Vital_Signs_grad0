"""Smoke validation for the DLinear/NLinear linear baselines.

Runs a capped number of batches on the real exported dataset for BOTH variants
and reports the 7 standard checks (epoch completions, loss decrease, NaN, GPU
footprint, params, forward latency, prediction range).

Usage:
  uv run python validate_linear_baseline_smoke.py

Note: the baselines are pure linear maps (no scan / attention), so batch 64 is
safe on the 4 GB laptop GPU.
"""
...
def _run_variant(name: str, model: torch.nn.Module, stats) -> None:
    print(f"\n================ {name} ================")
    n_params = count_parameters(model)
    print(f"[5] params: {n_params:,}")

    train_ds, val_ds, test_ds, _ = build_datasets(EXPORT, datasets=None)
...
def main() -> None:
    ...
    _, _, _, stats = build_datasets(EXPORT, datasets=None)
    dlinear = DLinearHeartRateModel(LinearBaselineConfig(variant="dlinear")).to(DEVICE)
    nlinear = NLinearHeartRateModel(LinearBaselineConfig(variant="nlinear")).to(DEVICE)
    _run_variant("DLinear", dlinear, stats)
    _run_variant("NLinear", nlinear, stats)
