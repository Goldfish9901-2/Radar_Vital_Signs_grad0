...
# Set in main() so subprocesses (run as file paths, not `-m`) can import `src`.
...
def run(cmd: List[str], cwd: Path | None = None, env: dict | None = None) -> None:
    ...
    if CODE_ROOT is not None:
        # Ensure `src` is importable even when the child runs a script file path.
        prev = full_env.get("PYTHONPATH", "")
        full_env["PYTHONPATH"] = f"{CODE_ROOT}{os.pathsep}{prev}" if prev else str(CODE_ROOT)
    if env:
        full_env.update(env)
