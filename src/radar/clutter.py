...
def clutter_temporal_highpass(cube: np.ndarray, cutoff: float = 0.1, **_: Any) -> np.ndarray:
    ...
    if n < 2:
        return cube
    # alpha chosen so the pole sits at `cutoff` Hz at the nominal 20 Hz frame rate.
    fs = 20.0
