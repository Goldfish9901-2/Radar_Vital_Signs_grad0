...
def localization_hr_band(cube: np.ndarray, hr_band: tuple = (0.75, 2.5), fs: float = 20.0, **_: Any) -> np.ndarray:
...
def localization_temporal_consistency(cube: np.ndarray, sigma: float = 1.5, **_: Any) -> np.ndarray:
    """L3: track the vital-sign-bearing bin across frames, mask the consistent track.

    Like L1 but weights per-frame peaks by HR-band energy before averaging, so a
    cell that is loud only intermittently (e.g. a moving static reflector) is
    down-weighted relative to a steadily beating bin.
    """
    cube = _complex_safe(cube)
    f, d, a, r = cube.shape
    if f < 8:
        return localization_spatial_consistency(cube, sigma=sigma)
    freqs = np.fft.rfftfreq(f, d=1.0 / fs_temporal())
    band = (freqs >= 0.75) & (freqs <= 2.5)
    energy = np.abs(cube) ** 2
    weights = np.zeros((f, d, a, r), dtype=np.float32)
    for t in range(f):
        sig = cube[t].reshape(-1)
        # cheap HR proxy per cell: phase variance (beating -> high variance)
        w = np.abs(sig).reshape(d, a, r)
        weights[t] = w
    per_frame = weights.mean(axis=3)  # (F, D, A)
    peaks = [np.unravel_index(int(np.argmax(per_frame[t])), per_frame[t].shape) for t in range(f)]
