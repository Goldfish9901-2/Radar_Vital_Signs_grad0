"""Smoke validation for TSMixer training (no architecture changes intended).
...
Usage:
  uv run python validate_tsmixer_smoke.py

Note: TSMixer is an all-MLP model (no scan / attention), so the batch size can
be the default 64 without OOM risk on the 4 GB laptop GPU.
"""
...
from src.models.tsmixer import TSMixerConfig, TSMixerHeartRateModel, count_parameters
...
def main() -> None:
    ...
    cfg = TSMixerConfig()  # use_frequency_domain defaults True
    model = TSMixerHeartRateModel(cfg).to(DEVICE)
